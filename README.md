...
+---------------------------------------------------------+
|                    Orchestrator                         |
|   (Центральный сервер, управляющий задачами)            |
|---------------------------------------------------------|
|                                                         |
| • GET  /internal/task — предоставляет задачу агенту     |
|                                                         |
| • POST /internal/task — принимает результаты вычислений |
+-------------------------------+-------------------------+
                                ^
                                |  POST (результаты вычислений)
                                |
                                |  GET (запрос задачи)
                                v
+---------------------------------------------------------+
|                        Agent                            |
|       (Компонент, исполняющий вычислительные задачи)    |
|---------------------------------------------------------|
| • Периодически запрашивает задачу через GET             |
|   /internal/task                                        |
|                                                         |
| • Выполняет вычисления                                  |
|                                                         |
| • Отправляет результат через POST /internal/task        |
+---------------------------------------------------------+
```
